...
namespace MQContract.CQRS.Interfaces
{
    public interface ICQRSConnection
    {
        ...
        ValueTask<TCommandResult?> ExecuteCommandAsync<TCommand, TCommandResult>(TCommand command, Context? context = null, CancellationToken cancellationToken = default)
            where TCommand : ICommand<TCommandResult>;
        ValueTask<TQueryResponse?> ExecuteQueryAsync<TQuery, TQueryResponse>(TQuery query, Context? context = null, CancellationToken cancellationToken = default)
            where TQuery : IQuery
